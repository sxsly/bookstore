...
<script type="text/javascript">
	...
	// 检查邮箱是否存在
    function checkEma() {
        $.post("${pageContext.request.contextPath}/user/checkemail.do",
           $("#email").serialize(), function (data) {
            if (data == "exist"){
                alert("该邮箱已经存在！不能重复注册！");
                $("#email").val("");
            }
        })
    }
    ...
    // 检查用户名是否存在
    function checkUname() {
        $.post("${pageContext.request.contextPath}/user/checkusername.do",
        $("#username").serialize(),function (data) {
                if (data == "exist"){
                    alert("该用户名已经被使用！");
                    $("#username").val("");
                }
...
<!-- 3.网上书城用户注册  start -->
	<div id="divcontent">
		<form action="${pageContext.request.contextPath}/user/userRegister.do" method="post" onsubmit="return checkForm();">
					...
					<td style="padding: 30px"><h1>新会员注册</h1>
						<table width="70%" border="0" cellspacing="2" class="upline">
							<tr>
								...
								<td style="width: 40%">
								<input type="text" class="textinput"  id="email" name="email" onkeyup="checkEmail();" onblur="checkEma()"/>
								</td>
								<td colspan="2"><span id="emailMsg"></span><font color="#999999">请输入有效的邮箱地址</font></td>
							</tr>
							<tr>
								<td style="text-align: right">会员名：</td>
								<td><input type="text" class="textinput"  id="username" name="username" onkeyup="checkUsername();" onblur="checkUname()"/>
								</td>
							...
							<tr>
								...
								<input type="text" class="textinput"
									style="width: 350px" name="telphone" />
								</td>
								<td>&nbsp;</td>
							</tr>
							<tr>
						...
						<table width="80%" border="0" cellspacing="2" class="upline">
							<tr>
								...
								<td style="width: 50%">
								<input type="text" class="textinput" name="checkWord" id="checkWord" required/>
								</td>
								<td>&nbsp;</td>
